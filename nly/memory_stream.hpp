...
/*
仅当 input 和 base 的前 byte_len 中的 bit 偏差数, 小于等于 allow_error_bit_count 时, 才返回 true
*/
...
public:
  ...
  /*
  功能: 将流向前移动 len 字节
  返回值: 实际移动的字节数(可能小于 len)
  注意: 若当前可用的数据小于 len, 则所有有效数据都将被移除
  */
  ...
  /*
  功能: 在流中寻找 target, 其长度为 target_byte, 允许的 bit 偏差是 allow_error_bit_count
  注意: 返回 -1 标识没有找到
  */
  const size_t find(
    const void*  target,
  ...
  /*
  功能: 从流的 offset_byte 处, 获取 need_byte 字节的数据, 存入 output
  返回值: 实际获取的字节数, 可能小于 need_byte
  注意: 此函数不会更改流的任何属性
  */
